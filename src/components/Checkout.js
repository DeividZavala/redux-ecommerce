...
function Checkout({doPayment, items, total, quantity}){
    ...
    return (
        <div style={{display:"flex", justifyContent:"center"}}>
            <div style={{border:"1px solid black", padding:50}} >
                ...
                <div>
                    Numero
                    <input className="uk-input" onChange={onChange} name="number" type="text" />
                    <br/>
                    Nombre
                    <input className="uk-input" onChange={onChange} name="name" type="text" />
                    <br/>
                    Fecha de expiración
                    ...
                    onFocus={onFocus}
                    onBlur={onBlur}
                    onChange={onChange} name="cvc" type="text" />
                    
                </div>
                <button 
                onClick={()=>doPayment(card)}
                    ...
                    {items.map((product, i)=>{
                        return (
                            <div style={{border:"1px solid black", padding:10}} key={i} >
                                ...
                                <p>{product.quantity} X ${product.price}MXN</p>
                            </div>
                        )
                    })}
                </div>
...
export default connect(mstp, {doPayment})(Checkout)